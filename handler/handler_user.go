...
func (h *userHandler) RegisterUser(c *gin.Context) {
	...
	formatter := user.NewUserFormat(newUser, token)
...
func (h *userHandler) Login(c *gin.Context) {
	...
	if err != nil {
		response := helper.ResponseAPI("Login Failed!", http.StatusBadRequest, "Failed", err.Error())
		c.JSON(http.StatusBadRequest, response)
		return
	}

	formatter := user.NewUserFormat(loginUser, token)
